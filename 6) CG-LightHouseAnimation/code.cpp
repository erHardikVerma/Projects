...
// Water parameters 
float waterHeight = 290.0;            // Height of water surface
float waveSpeed = 0.1;                // Speed of waves
float waveAmplitude = 10.0;           // Amplitude of waves
float waveFrequency = 0.025;          // Frequency of waves
float M_PI = 3.14159265358979323846;  //pi
float xpos = 500;                     // Starting position of Light
float d = 50;                         // Thickness of Light
bool moving_right = true;             //For movement of Light
int delay = 0;                        //Delay for Light on ends
int n = 10;                           //Number of Stars
float col[10];                        //Colour of Stars
bool isDayMode = true;
float boatx = 0.0;
...
void Land() {
    if (isDayMode) {
        glColor3f(0.78125,0.3162, 0.484);   //light Blue
    }
    else {
        glColor3f(0.4101, 0.164, 0.164);     //Dark Blue
...
void keyboard(unsigned char key, int x, int y) {
    ...
    case 'n': // Switch to night mode
        isDayMode = false;
        break;
    case 'a': // Switch to night mode
        boatleft();
        break;
    case 'd': // Switch to night mode
        boatright();
        break;
    }
}
